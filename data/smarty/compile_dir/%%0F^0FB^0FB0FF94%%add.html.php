<?php /* Smarty version 2.6.22, created on 2024-10-22 17:29:27
         compiled from unassigneddesign/add.html */ ?>
<?php require_once(SMARTY_CORE_DIR . 'core.load_plugins.php');
smarty_core_load_plugins(array('plugins' => array(array('modifier', 'date_format', 'unassigneddesign/add.html', 41, false),)), $this); ?>
...
">
    <input type="hidden" name="check" value="<?php echo $this->_tpl_vars['check']; ?>
">
...
" id="unf_customer_code" style="width:100px;" <?php if ($this->_tpl_vars['design']['customer_code']): ?>readonly<?php endif; ?>  class="sIE_No" onchange="customer_num();" />
...
" value="<?php echo $this->_tpl_vars['al']['design_attribute_value']; ?>
"/>
...
_choice" style="float:left; width:100%;">
                            <div id="<?php echo $this->_tpl_vars['al']['en_name1']; ?>
...
<script type="text/javascript">
    function add_row(name,attribute_row,attribute_id){
        ...
        attribute_row++;
        html='<div id="'+name+attribute_row+'" style="float:left; width:100%; margin-top:10px;">';
        html+='<div style="float:left; width:100%;"><select class="add01_select01" onchange="show_color(\''+name+'\','+attribute_row+',1)" id="'+name+'_select'+attribute_row+'" name="'+name+'[]" style="width:92%;"><option value=""></option>';
        <?php $_from = $this->_tpl_vars['attribute_list']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }if (count($_from)):
...
"><?php echo $this->_tpl_vars['lavi']['cn_name']; ?>
        ...
        <?php endif; ?>
        <?php endforeach; endif; unset($_from); ?>
        html+='</select><a href="javascript:void(0);" id="'+name+'_'+attribute_row+'" onclick="add_double_row(\''+name+'\','+attribute_row+','+attribute_id+')" style="font-size:26px; float:left; width:8%; line-height:26px;">+</a></div></div>';
...
/hotfix_quotation/add_customer',//通过Ajax取数据的目标页面
				...
				success:function(locals)//成功后执行的语句，这里是一个函数，"locals"是返回的数据
				{
                    if(locals.error){
                        ...
                        $("#unf_customer_code").val(locals.customer_code);
                        if(locals){
