<?php /* Smarty version 2.6.22, created on 2015-12-14 15:22:29
         compiled from cn_metrial_procure/metrial_procure_list.html */ ?>
...
"><?php echo $this->_tpl_vars['exc']['exchangerate_name']; ?>
          ...
          <div style="float:left; margin-bottom:5px;">
            ...
            <input type="text" class="sIE_No" id="tax_value" style="width:100px; text-align:right;"  name="tax_value" value=""  /><font style="float:left; line-height:26px;">%</font>
          </div>
...
"><?php echo $this->_tpl_vars['mt']['name']; ?>
          ...
          <div style="float:left; margin-bottom:5px;">
            ...
            <input type="text" class="sIE_No" disabled="disabled" style="width:45px; margin-left:5px;"  id="pd_unit" name="pd_unit" value="">
          </div>
          <!--<div style="float:left; margin-bottom:5px;">
...
?>
        <tr class="mp_tabClass01_m">
          <td><?php echo $this->_tpl_vars['pd']['material_name']; ?>
</td>
          <td ><?php echo $this->_tpl_vars['pd']['color_name']; ?>
</td>
...
"  onkeyup="need_num_change(<?php echo $this->_tpl_vars['pd']['id']; ?>
)" />
          <input type="hidden"  id="pd_old_quantity<?php echo $this->_tpl_vars['pd']['id']; ?>
" value="<?php echo $this->_tpl_vars['pd']['material_quantity']; ?>
"/></td>
          <td><?php echo $this->_tpl_vars['pd']['unit_name']; ?>
</td>
...
/cn_metrial_procure/need_num_change',//通过Ajax取数据的目标页面
		data:{"id":key,"pd_m_quantity":pd_material_quantity},
		...
		success:function(locals)//成功后执行的语句，这里是一个函数，"locals"是返回的数据
		{
			if(locals == 2){
				...
				$("#pd_material_quantity"+key).val('');
			}else{
				var result_price	=	eval('('+$("#total_amount").val()+"-"+pd_unit_price+"*"+$("#pd_old_quantity"+key).val()+ ')');	
				$("#pd_old_quantity"+key).val(pd_material_quantity);
				$("#total_amount").val(eval('('+ result_price+"+"+pd_material_quantity+"*"+pd_unit_price+')'));	
			}
		}
	});
}
...
/cn_metrial_procure/price_change',//通过Ajax取数据的目标页面
			data:{"id":key,"price":pd_unit_price},
			type:'post',//方法，还可以是"post"
			dataType:'text',//数据类型，还有其它的，详见jQuery手册
			success:function(locals)//成功后执行的语句，这里是一个函数，"locals"是返回的数据
			{
				if(locals == 2){
					alert("单价请填写正确的数字（注：不要带特殊符号）")
					$("#pd_unit_price"+key).val('');
				}else{
					var result_price	=	eval('('+$("#total_amount").val()+"-"+$("#pd_old_price"+key).val()+"*"+$("#pd_material_quantity"+key).val()+ ')');	
...
/cn_metrial_procure/get_pd_unit',//通过Ajax取数据的目标页面
			data:{'material_name':a,'material_color':color,'material_size':size},
			type:'get',//方法，还可以是"post"
			dataType:'json',//数据类型，还有其它的，详见jQuery手册
			success:function(locals)//成功后执行的语句，这里是一个函数，"locals"是返回的数据
			{
...
//		var pd_translated_unit2 = $("#mp_unit2").val();
		...
		var color_text = $("#mp_color_add_m").find("option:selected").text(); 
		var exchangerate = $("#exchangerate").val();
		var id_s = $("#id_s").val();
...
/cn_metrial_procure/insert_table_material',//通过Ajax取数据的目标页面
				type:'get',//方法，还可以是"post"
				data:{'material_name':material_name,'material_color':color,'material_size':size,'pd_quantity':pd_quantity,'pd_quantity_unit':pd_quantity_unit,'exchangerate':exchangerate,'ids':id_s},
				dataType:'json',//数据类型，还有其它的，详见jQuery手册
				success:function(locals)//成功后执行的语句，这里是一个函数，"locals"是返回的数据
				{
					locals=locals.split("|");
					if(locals[0] != 0){
					var table_value = "<tr class='mp_tabClass01_m'><td id=pd_name"+locals[0]+">"+material_name+"<input type='hidden' value='"+row_number+"' /><input type='hidden' name='materials["+row_number+"][material_name]' value='"+material_name+"' /></td><td id='pd_color"+locals[0]+"'>"+color_text+"<input type='hidden' name='materials["+row_number+"][color]' value='"+color+"' /></td><td id=pd_size"+locals[0]+">"+size_text+"<input type='hidden' name='materials["+row_number+"][size]' value='"+size+"' /></td> <td>"+pd_quantity+"<input type='hidden' name='materials["+row_number+"][pd_quantity]' value='"+pd_quantity+"' id='pd_material_quantity"+locals[0]+"' /></td><td>"+locals[1]+"<input type='hidden' name='materials["+row_number+"][pd_quantity_unit]' value='"+pd_quantity_unit+"' /></td><td><font class='exchangerate_character'>"+locals[2]+"</font><input type='text'  id='pd_unit_price"+locals[0]+"' name='materials["+row_number+"][pd_unit_price]' value='' onkeyup='supplier_price_change("+locals[0]+")' /><input type='hidden'  id='pd_old_price"+locals[0]+"' value='0'/></td><td onClick='getDel_add(this,"+locals[0]+")'><a href='#'>删除</a></td></tr>";
