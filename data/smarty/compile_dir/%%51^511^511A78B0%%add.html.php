<?php /* Smarty version 2.6.22, created on 2016-03-01 14:14:01
         compiled from cn_metrial_procure/add.html */ ?>
...
"  />
          <div style="float:left; margin-bottom:5px;">
            ...
            <input type="text" class="sIE_No" style="width:100px;"  name="procure_man" value="<?php echo $this->_tpl_vars['user_name']; ?>
"  />
          </div>
...
/cn_metrial_procure/get_exchangerate',//通过Ajax取数据的目标页面
	...
	$("#mp_add").click(function (){
		...
		if($("#insert_mp_add").val()!="" && $("#mp_color_add").val()!="" && $("#pd_quantity").val()!="" && $("#mp_size_add").val()!='' ){
			$.ajax({
				url:'<?php echo $this->_tpl_vars['__ROOT__']; ?>
/cn_metrial_procure/insert_table_material',//通过Ajax取数据的目标页面
				type:'get',//方法，还可以是"post"
				data:{'material_name':material_name,'material_color':color,'material_size':size,'pd_quantity':pd_quantity,'pd_quantity_unit':pd_quantity_unit,'exchangerate':exchangerate},
				dataType:'json',//数据类型，还有其它的，详见jQuery手册
				success:function(locals)//成功后执行的语句，这里是一个函数，"locals"是返回的数据
				{
					locals=locals.split("|");
