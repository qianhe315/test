<?php /* Smarty version 2.6.22, created on 2015-12-05 09:44:52
         compiled from unfinishdesign/add1.html */ ?>
...
"><?php echo $this->_tpl_vars['time']['time_name']; ?>
          ...
          <div style="float:left; margin-bottom:5px;">
            ...
            <input type="text" name="customer_code" id="unf_customer_code" style="width:100px;" value="<?php echo $this->_tpl_vars['customer_code']; ?>
...
<!--          <div style="float:left; margin-bottom:5px; ">
           ...
           <input type="hidden" id="image11" name="image11" value="" />
      <input type="hidden" id="image22" name="image22" value="" />
      <input type="hidden" name="huanse_status" value="2" />
...
"/>
          ...
          <div style="float:left; margin-bottom:5px;">
            <div class="sIE_l_01_title">原设计编号:</div>
            <input type="text" class="sIE_No" style="width:120px;"  name="yuan_design" id="yuan_design" value="" onkeyup="yuan_design_aa();" />
          </div>
          
            <input type="hidden" class="sIE_No" style="width:120px;"  name="aaa" id="aaa" value="" />
...
?>
      ...
      <!--<div>
      <form  method="post" enctype="multipart/form-data" action="<?php echo $this->_tpl_vars['__ROOT__']; ?>
/unfinishdesign/two_img_upload" id="two_img" target="yframe_unfinish">
          <div style="float:left; height:50px; width:80px; overflow:hidden; position:relative;">
             <input type="file" class="sIE_r_img_file" name="upload_file[]"  multiple="multiple" id="upload_file" style=" opacity:0; position:relative; z-index:10; margin-top:0px;"  />     
          ...
          <input type="submit" value="上传JPG和CDR" id="two_img_submit" style=" float:right; width:90px; height:24px;" /> 
      </form>
  </div>-->
    </div>
...
/unfinishdesign/upload_jpg" style="border:none; width:360px; height:400px;"></iframe>
        </div>
...
/unfinishdesign/check_photo',//通过Ajax取数据的目标页面
					...
					{*/
						//if(locals==1){
							...
							var code1 = $("#unf_customer_code").val();
							var code2 = $("#aaa").val();
							
							var image11 = $("#image11").val();
							var image22 = $("#image22").val();
...
function yuan_design_aa(){
	var yuan_design = $("#yuan_design").val();
	
	$.ajax({
		url:'<?php echo $this->_tpl_vars['__ROOT__']; ?>
/unfinishdesign/yuan_design_code',//通过Ajax取数据的目标页面
		data:{"yuan_design":yuan_design},
		type:'post',//方法，还可以是"post"
		dataType:'json',//数据类型，还有其它的，详见jQuery手册
		success:function(locals)//成功后执行的语句，这里是一个函数，"locals"是返回的数据
		{
			$("#aaa").val(locals.customer_code);
			$("#image111").html("<img width=360 src=" + locals.upload_jpg + ">");
			
			$("#image11").val(locals.upload_jpg);
			$("#image22").val(locals.upload_cdr);
			
			//alert(locals.upload_jpg)
		}								
