<?php /* Smarty version 2.6.22, created on 2016-01-08 14:06:45
         compiled from wrong_product_order/stock_edit.html */ ?>
...
function picture_select(){
	...
	if(document.getElementById('picture_select01_edit').value==1){
      ...
      <div class="stkInfEnter" style="width:98%;display:block;" layoutH="30" id="ttbjd_wsj">
    	  ...
    	  <div class="sIE_title" style="width:60%;">STOCK错误订单处理</div>
        </div>  	   
...
">-->
                <select class="picture_select02" id="gummed_paper_1" style="width:100px;" name="tapedata_id" onchange="gummed_paper()">
...
/hotfix_quotation/add_customer',//通过Ajax取数据的目标页面
              ...
              <div class="sIE_r" style="width:450px;">
                  ...
                  <?php if ($this->_tpl_vars['quotation_category_id'] == '3' || $this->_tpl_vars['quotation_category_id'] == '5'): ?>
...
"><?php echo $this->_tpl_vars['qsm']['price']; ?>
</td>
...
/hotfix_quotation/upload_jpg" style="border:none; width:260px; height:260px;"></iframe></div>
                    	<!--<div id="upload_name" style="float:left; margin-top:0px; font-size:14px; margin-left:10px;height:20px; width:200px;"><?php echo $this->_tpl_vars['jpg_name']; ?>
</div>-->
                    </div>     
...
/basemethod/get_material_c_infomation',//通过Ajax取数据的目标页面
	...
	$("#insert_material1_edit").click(function (){
		...
		$.ajax({
			url:'<?php echo $this->_tpl_vars['__ROOT__']; ?>
/wrong_product_order/insert_stock_quotation_sheet_material',//通过Ajax取数据的目标页面
			type:'get',//方法，还可以是"post"
			...
			success:function(locals)//成功后执行的语句，这里是一个函数，"locals"是返回的数据
			{
				if(locals != 0){
				locals=locals.split("|");
				var table_value = "<tr class='hq_tableClass02'> <td>"+material_name+"<input type='hidden' id='row_number' value='"+row_number+"' /><input type='hidden' id=sss_materialformula"+locals[0]+" value='"+locals[4]+"' /><input type='hidden' name='materials["+row_number+"][material_name]' value='"+material_name+"' /></td> <td id=w_h_id"+locals[0]+">"+size_width+"*"+size_height+" inch "+m_unit+"<input type='hidden' name='materials["+row_number+"][size_width]' value='"+size_width+"' /><input type='hidden' name='materials["+row_number+"][size_height]' value='"+size_height+"' /><input type='hidden' name='materials["+row_number+"][size]' value='"+size+"' /><input type='hidden' name='materials["+row_number+"][m_unit]' value='"+m_unit+"' /></td> <td>"+color+"<input type='hidden' name='materials["+row_number+"][color]' value='"+color+"' /></td> <td>"+quantity+"<input type='hidden' name='materials["+row_number+"][quantity]' value='"+quantity+"' /></td><td id=no_price_a_edit"+locals[0]+">"+locals[1]+"</td><td onClick='getDel(this,"+locals[0]+")'><a href='#'>删除</a></td></tr>";
...
function getDel(k,row){
   $(document).ready(function(){
	...
	$.ajax({
			url:'<?php echo $this->_tpl_vars['__ROOT__']; ?>
/wrong_product_order/delete_table_quotation_sheet_material',//通过Ajax取数据的目标页面
			type:'get',//方法，还可以是"post"
...
/*				no_price_a = no_price_a.replace("元","");
				...
				var no_final_quotation = no_total_price_id_a + "*" + no_piece;
				no_final_quotation = eval('('+no_final_quotation+')');
				no_final_quotation = no_final_quotation.toFixed(2);				
				$("#no_final_quotation_edit").val(aaa[0] + no_final_quotation);	*/
