<?php /* Smarty version 2.6.22, created on 2023-06-19 09:55:46
         compiled from stock_manage/add.html */ ?>
...
" <?php if ($this->_tpl_vars['stock_manage']['design_user_name'] == $this->_tpl_vars['qf']['user_name']): ?> selected="selected" <?php endif; ?> ><?php echo $this->_tpl_vars['qf']['user_name']; ?>
            ...
            <div class="sIE_l_01_title" style="width:80px; margin-top:10px;">STOCK 编号:</div>
...
"/>
            ...
            <div class="sIE_l_01_title" style="width:80px; margin-top:10px;">报价系数:</div>
            <div class="sIE_r01" style="width:180px;">
...
" />
            ...
            <div style=" float:right; margin-right:10px; margin-top:-230px; width:230px;">
                ...
                <textarea name="st_add_materials_by_hand" id="st_add_materials_by_hand" style="width:140px; height:140px;"></textarea>
                <button type="button" id="st_insert_materials_by_hand" style="margin-left:70px;">确定</button>
...
" class="pageForm" onsubmit="return validateCallback(this, navTabAjaxDone)" id="info_form_zhijia">
         ...
         <select name="stock_type1" id="stock_type1" style="width:137px; height:25px">
          ...
          <div class="sIE_l_01_title" style="width:80px; margin-top:5px;">设计人:</div>
            <div class="sIE_r01" style="width:180px;">
             <select name="design_user_name2" style=" margin-top:5px;width:137px; height:25px">
                  <option value="">请选择</option>
                  <?php $_from = $this->_tpl_vars['user_lists']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }if (count($_from)):
    foreach ($_from as $this->_tpl_vars['qf']):
?>
...
" class="pageForm" onsubmit="return validateCallback(this, navTabAjaxDone)" id="info_form_Tshirt">
                ...
                <div class="sIE_l_01_title" style="width:80px; margin-top:5px;">设计人:</div>
            <div class="sIE_r01" style="width:180px;">
             <select name="design_user_name3" style=" margin-top:5px;width:137px; height:25px">
                  <option value="">请选择</option>
                  <?php $_from = $this->_tpl_vars['user_lists']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }if (count($_from)):
    foreach ($_from as $this->_tpl_vars['qf']):
?>
...
?>
      ...
      <div class="sIE_r" style="width:420px; height:420px;" id="image0">
        
          <div class="sIE_r_img" style="height:400px; width:400px;" id="image_no_3">
...
/stock_manage/two_img_upload" id="stock_upload_img" target="yframe_stock" method="post" enctype="multipart/form-data" >
			  ...
			  <form style="float:left; width:260px;" id="st_upload_jpg" enctype="multipart/form-data">
                    	<input type="file" name="upload_file[]" value="上传" multiple="multiple"  style="position:relative; z-index:10; margin-top:0px;" />  
				</form>
				<button id="st_jpg_upload_submit">提交</button>
          </div>
...
/stock_manage/two_img_upload" style="border:none; width:400px; height:400px;"></iframe>
          ...
          <div style="float:left; margin-top:0px; width:400px;">
         <!-- <form action="<?php echo $this->_tpl_vars['__ROOT__']; ?>
/stock_manage/two_img_upload" id="stock_upload_jpg1" target="yframe_stock1" method="post" enctype="multipart/form-data" >
...
/stock_manage/check_photo',//通过Ajax取数据的目标页面
	...
	$("#submit_form").click(function () {
		...
		}else if(!validate($("#inch_w").val()) || !validate($("#inch_h").val()) || !validate($("#cm_w").val())|| !validate($("#cm_h").val()) || $("#inch_w").val() == ""  || $("#inch_w").val().indexOf('X')>=0 || $("#inch_h").val() == ""  || $("#inch_h").val().indexOf('X')>=0 || $("#cm_w").val() == "" || $("#cm_h").val() == ""){
...
/stock_manage/check_stock_code',
		...
		$("#st_jpg_upload_submit").click(function (){
		var formData = new FormData($('#st_upload_jpg')[0]);
        $.ajax({
			type: 'post',
			url:'<?php echo $this->_tpl_vars['__ROOT__']; ?>
/stock_manage/two_img_upload',          
			...
			dataType:'json', 
				...
				$.each(data,function(i,n){
				   if(i=='jpg'){
				   		$('#image_no_3').empty();
				   		$('#image_no_3').append('<a href="'+n+'"  target="_blank"><img src="'+n+'" width="280" height="300" /></a>');
				   }else if(i=='quotecol'){
				   		if( n!== null && n.indexOf("_") != -1){
				   			//$('#st_add_materials_by_hand').val(n.replace(/_/g,"\n"));
						}else{
							num=0;
						}
				   }else if(i=='width'){
				   		if( n!== null && n>0){
							if(n<25){
								//$('#inch_w').val(n.toFixed(2));	
								//$('#cm_w').val((n*2.54).toFixed(2));	
							}else{
								//$('#inch_w').val(n.toFixed(2)+'X');
								//$('#cm_w').val('NaN');
							}
				   		...
				   		if(n!== null && n>0){
							if(n<20){
								//$('#inch_h').val(n.toFixed(2));
								//$('#cm_h').val((n*2.54).toFixed(2));
							}else{
								//$('#inch_h').val(n.toFixed(2)+'X');
								//$('#cm_h').val('NaN');
							}
				 ...
				 if(num!=0){
					//$('#st_insert_materials_by_hand').trigger("click");
				}
	...
	//添加输入的材料
	$("#st_insert_materials_by_hand").click(function (){
		var str	=	$("#st_add_materials_by_hand").val();
		$("#text_err").html('');
...
/basemethod/insert_materials_of_stock',
				...
				success: function(data){
					if(data.err==1){
						...
						alert(data.text);
						$('#st_add_materials_by_hand').val();
						$('#st_add_materials_by_hand').val(data.item.replace(/_/g,"\n"));
						return;
					}else if(data.err==0){
