<?php /* Smarty version 2.6.22, created on 2024-09-03 09:19:09
         compiled from metrial_procure/edit.html */ ?>
...
" readonly="readonly" />
          ...
          <textarea class="add01_textarea" style="width:180px; margin-top:0px; margin-bottom:10px; height:80px;" name="note"><?php echo $this->_tpl_vars['metrial_procure']['note']; ?>
</textarea>
          </div>
...
"><?php echo $this->_tpl_vars['pd']['pd_color']; ?>
</td>
          <td>
          <?php if ($this->_tpl_vars['storage_status'] == '2'): ?>
          <input type="text" style="width:148px;" id="pd_supplier_color<?php echo $this->_tpl_vars['pd']['id']; ?>
" value="<?php echo $this->_tpl_vars['pd']['pd_supplier_color']; ?>
"  onkeyup="old_color_change(<?php echo $this->_tpl_vars['pd']['id']; ?>
)" />
          <?php else: ?>
          <input type="text" style="width:148px;" readonly="readonly" id="pd_supplier_color<?php echo $this->_tpl_vars['pd']['id']; ?>
" value="<?php echo $this->_tpl_vars['pd']['pd_supplier_color']; ?>
"  onkeyup="old_color_change(<?php echo $this->_tpl_vars['pd']['id']; ?>
)" />
          <?php endif; ?>
...
" onkeyup="price_change(<?php echo $this->_tpl_vars['pd']['id']; ?>
)" />
...
/*	alert(key);alert();*/
	   $(document).ready(function(){
		   ...
		   $.ajax({
				url:'<?php echo $this->_tpl_vars['__ROOT__']; ?>
/metrial_procure/old_color_change',//通过Ajax取数据的目标页面
				data:{"color":$("#pd_supplier_color"+key).val(),"id":key,"pd_name":$("#pd_name"+key).text(),"pd_color":$("#pd_color"+key).text(),"pd_size":$("#pd_size"+key).text(),"mp_code":$("#supplier_code").val()},
				type:'post',//方法，还可以是"post"
				dataType:'text',//数据类型，还有其它的，详见jQuery手册
	...
	function supplier_price_change(key){
	   $(document).ready(function(){
		   ...
		   $.ajax({
				url:'<?php echo $this->_tpl_vars['__ROOT__']; ?>
/metrial_procure/temporary_change',//通过Ajax取数据的目标页面
				data:{"color":$("#pd_supplier_color"+key).val(),"id":key,"price":pd_unit_price,"pd_name":$("#pd_name"+key).text(),"pd_color":$("#pd_color"+key).text(),"pd_size":$("#pd_size"+key).text(),"mp_code":$("#supplier_code").val(),'pd_m_quantity':pd_quantity},
				type:'post',//方法，还可以是"post"
				dataType:'text',//数据类型，还有其它的，详见jQuery手册
				success:function(locals)//成功后执行的语句，这里是一个函数，"locals"是返回的数据
				{
	...
	//修改原来材料 单价的js
	function price_change(key){
	   $(document).ready(function(){
		   var pd_unit_price = $("#pd_unit_price"+key).val();//单个金额
		   var pd_quantity	=	 $("#pd_q_"+key).val();//单个数量
		   var total_amount	=	$("#total_amount").val();//总金额
		   var all_price	=	$("#val_"+key).val();//单个材料原来的总价
		  // alert(all_price);return;
		   if(all_price < 0){
			   var last_price	=	eval('('+total_amount + all_price+ ')');//减去当前单条材料后的值
		   }
		   else{
		   ...
		   $.ajax({
				url:'<?php echo $this->_tpl_vars['__ROOT__']; ?>
/metrial_procure/change_old_price',//通过Ajax取数据的目标页面
