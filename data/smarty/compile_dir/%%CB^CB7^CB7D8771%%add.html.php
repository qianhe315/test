<?php /* Smarty version 2.6.22, created on 2015-06-24 10:24:10
         compiled from check_stock/add.html */ ?>
...
"><?php echo $this->_tpl_vars['mt']['material_name']; ?>
         ...
         <div style="float:left; margin-bottom:5px;">
            ...
            <input type="text" readonly="readonly" name="pd_quantity_unit" value="" id="pd_quantity_unit" style="height:21px;width:45px;margin-left:5px;" />
            <!--<select class="add01_select01" style=" width:50px; " id="pd_quantity_unit" name="pd_quantity_unit">
...
?>
              <option <?php if ($this->_tpl_vars['u1']['id'] == '5'): ?>selected="selected"<?php endif; ?>  value="<?php echo $this->_tpl_vars['u1']['name']; ?>
	...
	$("#material_add").click(function (){
		...
		if(pd_weight!=""){
		  if($("#material_name").val()!="" && $("#material_color").val()!="" && $("#pd_quantity").val()!="" && $("#price").val()!="" && a!=false && c!=false&& b!=false){
			  $.ajax({
				  url:'<?php echo $this->_tpl_vars['__ROOT__']; ?>
/check_stock/insert_table_design_material',//通过Ajax取数据的目标页面
				  type:'get',//方法，还可以是"post"
...
/check_stock/insert_table_design_material',//通过Ajax取数据的目标页面
				  type:'get',//方法，还可以是"post"
				  data:{'material_name':material_name,'material_color':color,'material_size':size,'pd_quantity':pd_quantity,'pd_quantity_unit':pd_quantity_unit,'pd_weight':pd_weight,'pd_weight_unit':pd_weight_unit},
				  dataType:'json',//数据类型，还有其它的，详见jQuery手册
				  success:function(locals)//成功后执行的语句，这里是一个函数，"locals"是返回的数据
				  {  
...
function getDel(k,row){
	...
	$.ajax({
			url:'<?php echo $this->_tpl_vars['__ROOT__']; ?>
/check_stock/delete_design_material',//通过Ajax取数据的目标页面
			type:'get',//方法，还可以是"post"
			data:{'id':row,'material_name':material_name,'material_color':color,'material_size':size,'pd_quantity':pd_quantity,'pd_quantity_unit':pd_quantity_unit,'pd_weight':pd_weight,'pd_weight_unit':pd_weight_unit},
			dataType:'json',//数据类型，还有其它的，详见jQuery手册
			success:function(locals)//成功后执行的语句，这里是一个函数，"locals"是返回的数据
			{
