<?php /* Smarty version 2.6.22, created on 2016-01-13 10:17:03
         compiled from invoice_szo/index.html */ ?>
...
/invoice_szo/xls_export" method="post" id="invoice_export">
...
/invoice_szo/bb/" target="selectedTodo" id="del" rel="group_ids_invoice[]" postType="string" title="确定生成销售明细?"><span>添加到销售明细</span></a></li>            
             <li class="line">line</li>
...
/invoice_szo/add_freight-id-<?php echo $this->_tpl_vars['dis']['id']; ?>
.html" class="btnAdd"></a>
                    <?php endif; ?>
                   <?php if ($this->_tpl_vars['dis']['sales_status'] == '1'): ?>
                   		<a target="dialog" title="添加折扣" rel="" href="<?php echo $this->_tpl_vars['__ROOT__']; ?>
/invoice_szo/add_discount-id-<?php echo $this->_tpl_vars['dis']['id']; ?>
.html" class="btnAdd"></a>
                    <?php endif; ?>  
...
/invoice_szo/special_invoice_approval-id-<?php echo $this->_tpl_vars['dis']['id']; ?>
.html" class="btnEdit"></a>
                    <?php endif; ?>              
                    <?php if ($this->_tpl_vars['dis']['sales_status'] == '1' && $this->_tpl_vars['dis']['status'] != '2'): ?>
                        <a title="删除吗？" target="ajaxTodo" href="<?php echo $this->_tpl_vars['__ROOT__']; ?>
/invoice_szo/del-id-<?php echo $this->_tpl_vars['dis']['id']; ?>
...
/invoice_szo/sale_detail-'+p,//通过Ajax取数据的目标页面
								data:p,
...
/invoice_szo/create_invoice_china-'+p);
							}
						}
					}
						...
						for(var i=0;i<saleman_name01.length;i++){
							...
							if(saleman_name01[i]){
								if(saleman_name01[i]!= $("#saleman_id").val() && $("#saleman_id").val() != 'admin'){
									...
									$.ajax({
										url:'<?php echo $this->_tpl_vars['__ROOT__']; ?>
/invoice_szo/fp_duo',//通过Ajax取数据的目标页面
										data:{'id':check_arr},			
										type:'get',//方法，还可以是"post"
										...
										success:function(locals)//成功后执行的语句，这里是一个函数，"locals"是返回的数据
										{
											...
											if(locals == 1){
												window.open('<?php echo $this->_tpl_vars['__ROOT__']; ?>
/invoice_szo/create_invoice_usa_no_image-'+p);
											}else if(locals == 3){
												
												window.open('<?php echo $this->_tpl_vars['__ROOT__']; ?>
/invoice_szo/create_invoice_usa_no_image_materials-'+p);
											}
											else{
...
/invoice_szo/fp',//通过Ajax取数据的目标页面
										...
										success:function(locals)//成功后执行的语句，这里是一个函数，"locals"是返回的数据
										{
											...
											if(locals == 1){
												window.open('<?php echo $this->_tpl_vars['__ROOT__']; ?>
/invoice_szo/create_invoice_usa-'+p);
											}else if(locals == 3){
												
												window.open('<?php echo $this->_tpl_vars['__ROOT__']; ?>
/invoice_szo/create_invoice_usa_materials-'+p);
											}
											else{
												alert("客户信息不完整,不可生成发票！");
											}
		...
		//var touid = $("#newtouid").val();
			...
			if(check_arr ==''){
				alert('必须选择客户');
				  ...
				  $.ajax({
					  url:'<?php echo $this->_tpl_vars['__ROOT__']; ?>
/invoice_szo/fp_review',//通过Ajax取数据的目标页面
					  data:{'id':check_arr},			
					  type:'get',//方法，还可以是"post"
					  dataType:'json',//数据类型，还有其它的，详见jQuery手册
					  success:function(locals)//成功后执行的语句，这里是一个函数，"locals"是返回的数据
					  {
	                     ...
	                     window.open('<?php echo $this->_tpl_vars['__ROOT__']; ?>
/invoice_szo/create_invoice_usa_review-'+p);
						  }else{
							  alert("客户代码不统一！");
						  }
										}
