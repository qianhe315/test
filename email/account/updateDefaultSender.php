<?php


/**
 * 更新默认发件人
 */
date_default_timezone_set("PRC");

$prikey = '-----BEGIN RSA PRIVATE KEY-----

-----END RSA PRIVATE KEY-----';

$account_name = "zhangsan";
$domain = "abc.com";
$product = "abc_com";
$sender = "zs@abc.com";
$time = date(time()) . '000';

$res = openssl_pkey_get_private($prikey);
$src = "account_name=" . $account_name . "&domain=" . $domain . "&product=" . $product . "&sender=" . $sender . "&time=" . $time;

if (openssl_sign($src, $out, $res)) {
	$sign = bin2hex($out);
	$url = "http://apibj.qiye.163.com/qiyeservice/api/account/updateDefaultSender";
	$pram = $src . "&sign=" . $sign;

	echo $url . "?" . $pram . "<br>";

	$ch = curl_init();
	curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
	curl_setopt($ch, CURLOPT_URL, $url);
	curl_setopt($ch, CURLOPT_POST, 1);
	curl_setopt($ch, CURLOPT_POSTFIELDS, $pram);

	$data = curl_exec($ch);
	echo $data;
	curl_close($ch);
}
?>