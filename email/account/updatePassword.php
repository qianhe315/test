<?php


/**
 * 更新帐号密码
 */
date_default_timezone_set("PRC");

$prikey = '-----BEGIN RSA PRIVATE KEY-----

-----END RSA PRIVATE KEY-----';

$account_name = "zhangsan";
$domain = "abc.com";
$pass_type = "1";
$passchange_req = "0";
$password = md5("Test135@");
$product = "abc_com";
$time = date(time()) . '000';

$res = openssl_pkey_get_private($prikey);
$src = "account_name=" . $account_name . "&domain=" . $domain . "&pass_type=" . $pass_type . "&passchange_req=" . $passchange_req . "&password=" . $password . "&product=" . $product  . "&time=" . $time;

if (openssl_sign($src, $out, $res)) {
	$sign = bin2hex($out);
	$url = "http://apibj.qiye.163.com/qiyeservice/api/account/updatePassword";
	$pram = $src . "&sign=" . $sign;

	echo $url . "?" . $pram . "<br>";

	$ch = curl_init();
	curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
	curl_setopt($ch, CURLOPT_URL, $url);
	curl_setopt($ch, CURLOPT_POST, 1);
	curl_setopt($ch, CURLOPT_POSTFIELDS, $pram);

	$data = curl_exec($ch);
	echo $data;
	curl_close($ch);
}
?>