<?php 
//私钥，请使用企业自己生成的私钥
$prikey = '-----BEGIN RSA PRIVATE KEY-----

-----END RSA PRIVATE KEY-----';
//域名，请使用企业自己域名
$domain = "cstown.com";
//登录账号，如需单点登录到 test001@test.com，只填写@前信息即可
$account_name = "service";

$time = date(time())* 1000;
//语言，0-中文，1-英文，可以不传此参数，默认为0
$lang = "0";
$src = 'domain=cstown.com&product=cstown_com&time='.$time;
$res = openssl_pkey_get_private($prikey);
if (openssl_sign($src, $out, $res))
{
	$enc = bin2hex($out);
}
?>



<form action="https://apibj.qiye.163.com/qiyeservice/api/unit/getUnitList?domain=cstown.com&product=cstown_com&time=<?php echo $time;?>&sign=<?php echo $enc; ?>" method="post">

	<input type="submit" name="submit" value="提交" />
</form>
