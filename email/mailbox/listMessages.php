...
$params = '{"fids":[1,5],"order":"receivedDate","desc":true,"start":0,"limit":5,"returnTotal":true}';

$product = "abc_com";
$time = date(time()) . '000';

$res = openssl_pkey_get_private($prikey);
$src = "account_name=" . $account_name . "&domain=" . $domain . "&params=" . $params . "&product=" . $product . "&time=" . $time;

if (openssl_sign($src, $out, $res)) {
	$sign = bin2hex($out);
	$url = "http://apibj.qiye.163.com/qiyeservice/api/mailbox/listMessages";
	$pram = $src . "&sign=" . $sign;
