...
$src = "account_name=" . $account_name . "&domain=" . $domain . "&mobile=" . $mobile . "&product=" . $product . "&status=" . $status . "&time=" . $time;

if (openssl_sign($src, $out, $res)) {
	$sign = bin2hex($out);
	$url = "http://apibj.qiye.163.com/qiyeservice/api/mobile/bindSms";
	$pram = $src . "&sign=" . $sign;
