<?php


/**
 * 设置部门排序值
 */
date_default_timezone_set("PRC");

$prikey = '-----BEGIN RSA PRIVATE KEY-----

-----END RSA PRIVATE KEY-----';

$domain = "abc.com";
$product = "abc_com";
$rank = "1";
$time = date(time()) . '000';
$unit_id = "667454";

$res = openssl_pkey_get_private($prikey);
$src = "domain=" . $domain . "&product=" . $product . "&rank=" . $rank . "&time=" . $time . "&unit_id=" . $unit_id;

if (openssl_sign($src, $out, $res)) {
	$sign = bin2hex($out);
	$url = "http://apibj.qiye.163.com/qiyeservice/api/unit/updateUnitRank";
	$pram = "domain=" . $domain . "&product=" . $product . "&rank=" . $rank . "&time=" . $time . "&unit_id=" . $unit_id . "&sign=" . $sign;

	echo $url . "?" . $pram . "<br>";

	$ch = curl_init();
	curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
	curl_setopt($ch, CURLOPT_URL, $url);
	curl_setopt($ch, CURLOPT_POST, 1);
	curl_setopt($ch, CURLOPT_POSTFIELDS, $pram);

	$data = curl_exec($ch);
	echo $data;
	curl_close($ch);
}
?>